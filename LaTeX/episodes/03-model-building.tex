...
\begin{itemize}
  ...
  \item El repositorio de ejemplos incluye `simulateAmber.py` + `simulateCharmm.py`, y `argon-chemical-potential.py` para verificar energías libres en líquidos simples.
...
\begin{itemize}
  \item \texttt{coarse\_grained\_polymer.py} (OpenMM Cookbook) crea topologías bead-spring que muestran cómo definir masas y enlaces manualmente.
  \item Ese mismo enfoque se conecta con el script `argon-chemical-potential.py`, donde se parametrizan fuerzas de Lennard-Jones y se mide energía libre de inserción.
...
\begin{itemize}
  ...
  \item Si hay membranas, usar `Modeller.addMembrane` y luego `simulateAmber.py` con barostato anisotrópico.
