...
\begin{itemize}
  \item Cargar estructuras PDB/GROMACS/AMBER/CHARMM/TINKER desde los scripts oficiales (p.ej., `simulatePdb.py`, `simulateGromacs.py`, `simulateAmber.py` y `simulateCharmm.py`).
  ...
  \item Ejecutar integraciones con pasos definidos desde la capa de aplicaci\'on y guardar checkpoints con la rutina de OpenMM-Setup (User Guide §3.5-3.15).
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item `simulateAmber.py` acepta `.prmtop` + `.inpcrd`, mientras que `simulateCharmm.py` y `simulateGromacs.py` respetan sus formatos nativos.
  \item `simulatePdb.py` es el punto de partida tradicional para dip\u00e9ptido de alanina o prote\'inas-ligando sin parametrizar.
